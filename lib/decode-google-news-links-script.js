(async () => {
    ...
    function decodeGoogleNewsUrlResponse(response) {
    ...
    const allLinks = Array.from(document.querySelectorAll('a').values());

    await Promise.all(allLinks.filter((n) => n.href && n.href.includes('/read')).map(async (n) => {
        const decoded = await decodeGoogleNewsUrl(n.href);
        n.href = decoded;
    }));

    document.querySelector('body').id = "done-decoding-links"
